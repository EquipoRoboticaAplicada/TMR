...
def run_debug(zed: ZEDShared, vision: VisionZED, odo=None):
    ...
    cv.namedWindow("DEBUG ZED", cv.WINDOW_NORMAL)
    cv.resizeWindow("DEBUG ZED", 672, 376)
    ...
    while True:
        ...
        if draw_frame is None and detected and cx is not None and cy is not None:
            cv.circle(frame, (cx, cy), 6, COLOR_BOX, -1)
            cv.line(frame, (cx_c, cy_c), (cx, cy), COLOR_BOX, 1)
            dist_txt = f"{dist:.2f} m" if dist is not None else "dist: N/A"
            cv.putText(frame, label,                (cx+10, cy-10), FONT, 0.6, COLOR_BOX,  2)
            cv.putText(frame, dist_txt,             (cx+10, cy+15), FONT, 0.6, COLOR_INFO, 2)
            cv.putText(frame, f"area: {int(area)}", (cx+10, cy+38), FONT, 0.5, COLOR_INFO, 1)

        # ── HUD ────────────────────────────────────────────────────────
        det_color = (0, 255, 0) if detected else (0, 0, 255)
        cv.putText(frame, f"Detectado: {'SI' if detected else 'NO'}",
